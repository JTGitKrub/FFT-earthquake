...
def compute_hvsr(ns, ew, ud, fs, window="hann", smooth_b=40.0, one_sided=True):
    """HVSR = sqrt(FFT_NS^2 + FFT_EW^2) / FFT_UD"""
...
def compute_arias(data, fs):
    """Arias Intensity Ia(t) = π/(2g) ∫a²dt"""
    g = 9.81
    dt = 1.0 / fs
    a2 = data ** 2
...
def _smart_genfromtxt(path):
    """Auto-detect delimiter (comma or whitespace), skip string header rows"""
    # detect delimiter
    ...
    if raw.ndim == 1:
        # try comma if whitespace failed
...
class FileImportDialog(QWidget):
    """Pop-up ให้ user กรอก dt และ start column"""
    ...
    def __init__(self, filename, ncols, parent=None):
        ...
        sep.setStyleSheet(f"background:{BORDER}; max-highe:1px"); 
        lay.addWidget(sep)
        ...
        # Row: spinbox + Hz label side by side
        ...
        self.hz_lbl.setStyleSheet(
            f"color:{ACCENT}; font-size:16px; font-weight:bold; padding-lef:8px")
        ...
        v1.addLayout(row_dt)
        lay.addWidget(g1)
        #for w in (lbl_dt, self.spin_dt, self.hz_lbl): v1.addWidget(w)
        #lay.addWidget(g1)
        ...
        #for w in (lbl_col, self.spin_col): v2.addWidget(w)
        #lay.addWidget(g2)
...
class ComponentFilterPanel(QGroupBox):
    """HP/LP + IFFT panel สำหรับ 1 component"""
    def __init__(self, comp_name, color, parent=None):
        ...
        self._fs       = None
        self._t        = None
        self._modified = None   # IFFT result
        self._build()
    ...
    def _run_ifft(self):
        ...
        win.setStyleSheet(STYLESHEET); win.resize(1000, 580)
        plt.rcParams.update(MPLSTYLE)
        ...
        ax1.plot(self._t, self._data,  color=self.color, lw=0.8, alpha=0.7, label="Original")
        ax1.plot(self._t, data_filt,   color=ACCENT2,    lw=0.9, label="Modified")
        ...
        ax2.set_title(f"Modified  ({' + '.join(parts)})", color=ACCENT2, pad=6)
        ...
        fig.text(0.5,0.005,
            ...
            f"RMS: {np.sqrt(np.mean(self._data**2)):.5f} → {np.sqrt(np.mean(data_filt**2)):.5f}",
        ...
        canvas.draw(); win.show()
        self._ifft_win = win
        self.lbl_status.setText(f"✓ Modified waveform ready  PGA {pga_f:.5f}")

    def get_modified(self):
        """คืน modified data หรือ original ถ้ายังไม่ได้ filter"""
        return self._modified if self._modified is not None else self._data


# ════════════════════════════════════════════════════════════════════════════
#  Tab 1 — FFT (redesigned)
# ════════════════════════════════════════════════════════════════════════════

class FFTTab(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._raw      = None   # full raw array
        self._dt       = 0.005
        self._acc_col  = 1
        self._has_time = False
        self._ncols    = 0
        self._mode     = "single"  # "single" or "3comp"
        self._comp_data = {}       # {"EW":arr, "NS":arr, "V":arr} or {"ACC":arr}
        self._t        = None
        self._build()
    ...
    def _build(self):
        ...
        # Export modified waveform (3-comp CSV)
        ...
        # ── Right: scroll area for dynamic plots ────────────────────────────
        ...
        # Filter panels (created dynamically after load)
        self._filter_panels = {}   # comp_name -> ComponentFilterPanel
    ...
    def _rebuild_right_panel(self, fs, dt):
        ...
        for comp, data in self._comp_data.items():
            ...
            # FFT placeholder (will be filled on Compute FFT)
...
class AccVelDispTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._fs    = None
        self._comp_data  = {}   # {"EW": arr, "NS": arr, "V": arr} or {"ACC": arr}
        self._comp_results = {}  # comp -> {"acc","vel","disp"}
        self._build()
    ...
    def _build(self):
        ...
        self.cb_unit.setCurrentIndex(1) #disable จนกว่าจะเลือก Custom
    ...
    def _load(self):
        ...
        try:
            ...
            self._raw = raw
            ncols = raw.shape[1]
            # update cb_comp options based on ncols
            self.cb_comp.clear()
            if ncols >= 3:
                # assume col 0+ = EW,NS,V (or time+EW+NS+V if 4+)
    ...
    def run(self):
        ...
        idx = self.cb_input.currentIndex()  # 0=Acc, 1=Vel, 2=Disp
        self._comp_results = {}
        stats_lines = []
    ...
    def _save_selected_type(self):
        ...
        sig_key = self.cb_exp_type.currentText().lower()  # acceleration/velocity/displacement
